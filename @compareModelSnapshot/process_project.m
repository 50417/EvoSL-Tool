function process_project(obj)
    child_parent_version_sha = obj.child_parent_version_sha;    
     ...
     total_project_snapshots = length(child_parent_version_sha);
     obj.WriteLog(sprintf('Total Number of Project Snapshots = %d',total_project_snapshots));
     
     processing_folder_sql = ['SELECT  DISTINCT before_project_sha, after_project_sha, model from ' char(obj.table_name) ];
     processed_project_model_commits = utils.combine_each_row_to_str(fetch(obj.conn,processing_folder_sql),'_');
     for i = 1:total_project_snapshots
        obj.WriteLog(sprintf("============================Processing Snapshot %d=======================",i));
       ...
       % if ~isempty(processed_project_model_commits) && sum(contains(processed_project_model_commits,child_parent_version_sha{i,1})) > 0
        ...
        obj.WriteLog(sprintf('======= Processed Snapshot %d ========= ',i));
